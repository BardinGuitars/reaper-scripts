...
-- Полная версия с улучшениями читаемости + Containers (1 уровень)
...
<div class="summary">
  ...
  <div class="summary-tracks">Треки с отличиями: ]] .. (#track_list > 0 and table.concat(track_list, ", ") or "—") .. [[</div>
</div>
]]
    ...
    if #sec_rows > 0 then
      ...
      for _, row in ipairs(sec_rows) do
        if row.track ~= last_track then
          f:write('<tr class="track-group"><td colspan="' .. (4 + #project_names) .. '">▶ ' .. row.track .. '</td></tr>\n')
        ...
        f:write('<tr class="' .. row_class .. '" data-track="' .. row.track:lower() .. '" data-plugin="' .. (row.short_name or ""):lower() .. '">\n')

        -- Колонка плагина
        if sec == "PLUGINS" then
          local display = row.short_name or row.param
          local title = row.full_name or ""
          f:write('<td class="plugin" title="' .. title:gsub('"', '&quot;') .. '">' .. display .. '</td>\n')
        else
          f:write('<td></td>\n')
        end

        f:write('<td></td>\n')
        f:write('<td class="track">' .. row.track .. '</td>\n')
        ...
        f:write('<td class="param">' .. param_display .. '</td>\n')
...
function copyDiffs() {
  ...
  text += 'Projects: ]] .. table.concat(project_names, ' | ') .. [[\\n\\n';
